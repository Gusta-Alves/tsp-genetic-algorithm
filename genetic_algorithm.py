...
def nearest_neighbor_heuristic(cities_locations: List[Tuple[float, float]], start_city_index: int = 0) -> List[Tuple[float, float]]:
...
def convex_hull_heuristic(cities_locations: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    """
    ...
    for point in interior_points:
        ...
        for i in range(len(tour)):
            ...
            new_distance = calculate_distance(tour[i], point) + calculate_distance(point, tour[(i + 1) % len(tour)])
...
def tournament_selection(population: List[List[Tuple[float, float]]], 
                        population_fitness: List[float], 
                        tournament_size: int = 3) -> List[Tuple[float, float]]:
